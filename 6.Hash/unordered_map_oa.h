#ifndef _CP_UNORDERED_MAP_OA_INCLUDED_
#define _CP_UNORDERED_MAP_OA_INCLUDED_
#include <stdexcept>
#include <iostream>
#include <iomanip>
#include <functional>
...
namespace CP {
    ...
    class unordered_map {
        ...
        public :
            ...
            // Constructor
            unordered_map() : mBuckets(std::vector<BucketT>(11)), mSize(0), mHasher(HasherT()),
                    mEqual(EqualT()), mMaxLoadFactor(0.7), mUsed(0), mNextAddress(NextAddressT()) {
            }
            unordered_map(const unordered_map<KeyT, MappedT, HasherT, EqualT, NextAddressT>& other) :
                    mBuckets(other.mBuckets), mSize(other.mSize), mHasher(other.mHasher), mEqual(other.mEqual),
                    mMaxLoadFactor(other.mMaxLoadFactor), mUsed(other.mUsed), mNextAddress(other.mNextAddress) {
            }
            ~unordered_map() {
                clear();
            }
            // Copy Assignment
            unordered_map<KeyT, MappedT, HasherT, EqualT, NextAddressT>& operator=(unordered_map<KeyT, MappedT, HasherT, EqualT, NextAddressT> other) {
                using std::swap;
            ...
            MappedT& operator[] (const KeyT& key) {
                ...
                if (mBuckets[pos].available()) { // If not found, Create new
                    insert_to_position(std::make_pair(key, MappedT()), pos);
                }
                return mBuckets[pos].value.second;
            }
            // Modifier
            void rehash(size_t m) {
                if (load_factor() <= max_load_factor() && m <= mBuckets.size()) {
                    return;
                }
                m = std::max(m, (size_t) mBuckets.size()*2);
                ...
                this->clear();
                mBuckets.resize(m); // vector method
                for (auto& val : temp) {
                    this->insert(val);
                }
            }
            ...
            std::pair<iterator, bool> insert(const ValueT& val) {
                ...
                if (mBuckets[pos].available()) {
                    ...
                    result.second = true;
                } else { // if key same can not insert
            ...
            // For Debug
            bool iBucket_size(size_t i) {
                return mBuckets[i].size();
            }
            void print() {
                using namespace std;
                std::cout << "------------------------------------------" << std::endl;
                ...
                for (size_t i = 0;i < bucket_count();i++) {
                    if (bucket_count() > 0) {
                        ...
                        for (size_t j= 0;j < iBucket_size(i);j++) {
...
#endif // _CP_UNORDERED_MAP_OA_INCLUDED_
